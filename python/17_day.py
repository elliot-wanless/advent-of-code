...
while queue:
    ...
    # If we've visited this block before, skip
    ...
    if count < 10 and (r_dir, c_dir) != (0, 0):
        new_r = r + r_dir
        new_c = c + c_dir
        # If the new row and column aren't out of bounds, add the block to the queue
    ...
    if count >= 4 or (r_dir, c_dir) == (0, 0):
        for new_r_dir, new_c_dir in [(0, 1), (1, 0), (0, -1), (-1, 0)]:
            # If the new direction isn't the same or doubling back on the path, add the block to the queue
            if (new_r_dir, new_c_dir) != (r_dir, c_dir) and (new_r_dir, new_c_dir) != (-r_dir, -c_dir):
                new_r = r + new_r_dir
                new_c = c + new_c_dir
                # If not out of bounds, add the block to the queue with count of 1 (as new direction started)
                if 0 <= new_r < len(grid) and 0 <= new_c < len(grid[0]):
                    extra_loss = grid[new_r][new_c]
